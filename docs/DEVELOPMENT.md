...
JEV 未确认的区域交给 DeepSeek 复核，仍只接受网页已有候选 ID。结构仍不确定时，扩大对应区域的文字候选范围重试一次；仍不完整则停止答题和翻页。JEV 结构判断阈值暂定为 `0.85`，尚未通过真实页面评测校准。

确认后的对应关系缓存在当前文档中。同一局部布局的自定义选项组复用已确认的题干和标签位置，每题重新读取可见文字；题干、选项或进度变化时生成新的题目身份，每题仍独立调用模型作答。布局变化、文本缺失或选项重复时重新识别结构。异步模型结果返回后也重新检查来源，避免旧结果绑定到新题目。
...
需要先「检查答案」才出现下一题的页面，会在答案填写完成后检查并等待下一步；无响应时停止重复点击。检查后锁定的已选单选组仍可读取，不会重复作答。「完成测验」等最终提交按钮继续受提交开关控制。
